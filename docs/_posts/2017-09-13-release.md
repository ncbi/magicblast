...
* Reads from NCBI SRA repository are downloaded from the cloud when ```-sra``` or ```-sra_batch``` option is used. [More details here]({{ site.baseurl }}{% link _posts/2021-05-06-cloud-sra.md %}).
