...
* Magic-BLAST now reports Mapping Quality (MAPQ) score in the SAM output. The scores are 60 for a single alignment, and $$ \begin{displymath} \lfloor -10 \log_{10}(1 - 1 / num_alignments) + 0.5 \rfloor \end{displaymath} $$ for more than one alignment.
